...
async function listForChannel(channelLogin, config) {
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      action,
      // Наказание по вердикту растёт по кривой отношения, а назначенное кодом на дне - нет:
      // вердикт это суждение, а дно вывод, и множитель полагается только первому. Колонка обещает
      // худший случай, поэтому считает по вердикту.
      ...
      // Сколько раз этого человека уже прощали. Частоту прощения код не ограничивает - решает
      // модель, - и счётчик здесь единственное место, где видно, не превратилось ли «извини» в
      // способ обнулять счёт по расписанию.
      pardonCount: Math.max(0, parseInt(row.pardonCount, 10) || 0),
      pardonedAt: row.pardonedAt || null,
      pardonedBy: row.pardonedBy || null,
    };
  });
}
