...
router.get("/admin/ai", requireAdmin, async (req, res, next) => {
  try {
    ...
    res.render("adminAi", {
      ...
      // показан на странице (свой, а если своего нет - встроенный), поэтому «отличий нет» здесь
      // означает то же, что и подпись над ним. Без опубликованного ботом встроенного текста
