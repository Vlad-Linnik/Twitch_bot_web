...
const KIND_MAX_WIDTH = {
  backdrop: 1920,
  medallion: 512,
  // Steam serves showcase art at 96px; 128 leaves room for a denser screen without inventing
  // detail that was never in the source.
  showcase: 128,
};

const REMOTE_TIMEOUT_MS = 8000;
// A showcase tile is a few KB. Anything wildly larger is not the artwork we asked for, and the
// point of a cap on a REMOTE fetch is to bound it before it is in memory.
...
// Downloads one image and stores it exactly like an upload. The caller is responsible for
// deciding the URL is safe to fetch (lib/steamShowcase.js's isAllowedImageUrl) - this function
// will not second-guess it, so never hand it a URL that came straight from a form.
//
// Re-hosting rather than hotlinking is deliberate. The site's CSP allows images from 'self' plus
// two named CDNs, and widening it to a third party for one page's decoration is a worse trade
// than storing a handful of kilobytes: it also means the case keeps rendering when Steam rotates
// a CDN hostname or the artwork disappears.
async function saveRemoteImage(url, kind) {
  ...
  if (buffer.length > MAX_REMOTE_BYTES) throw new Error("image too large");
  return saveThemeImage(buffer, kind);
}
...
module.exports = { saveThemeImage, saveRemoteImage, themeImageBytes, deleteThemeImage, KIND_MAX_WIDTH };
