...
// Поднято с 20 по требованию владельца: на живом чате #mistercop двадцати исключающих слов
// не хватало. Цена измерена - постороннее сообщение со 100 исключениями разбирается за
// 0,067 мс, потому что обязательные слова проверяются первыми и отсеивают его до списка.
const MAX_WORDS = 100;
