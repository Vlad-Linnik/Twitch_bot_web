...
// Vector = the room drawn as SVG/CSS, photo = a wallpaper behind it. In photo mode the
...
// Shipped wallpapers, per skin - and per skin for the same reason the palettes are: a gilded
// state room behind a bower of roses is not a variation on it, it is the wrong room. Same
// "first entry is the default and the cross-skin fallback" rule as ACCENT_PRESETS.
const BACKDROP_PRESETS = {
  throne: ["hall", "ceiling", "colonnade"],
  rose: ["blossom", "peonies", "garden"],
};

function backdropPresetsForTheme(theme) {
  return BACKDROP_PRESETS[theme] || BACKDROP_PRESETS[THEMES[0]];
}
...
function resolveTheme(doc) {
  ...
  const backdrops = backdropPresetsForTheme(theme);
  return {
    ...
    backdrop: {
      ...
      preset: pickFromList(doc?.backdrop?.preset, backdrops, backdrops[0]),
...
function parseSubmittedTheme(body, existing) {
  ...
  return {
    ...
    backdrop: {
      ...
      preset: pickFromList(body["backdrop.preset"], backdrops, backdrops[0]),
