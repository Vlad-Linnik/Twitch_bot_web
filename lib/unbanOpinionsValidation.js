...
// own header).
//
// WHAT WRITES THIS. Two callers, and this validator is deliberately the only way in for both:
//   - routes/unbanBureau.js's "заказать разбор" button, which generates the speeches here and now
//     via lib/unbanOpinionsGenerator.js (Gemini first, Anthropic only when that fails);
//   - routes/adminApi.js's PUT .../opinions, the original path, still used by the local driver
//     scripts/local/opinions.js for bulk or by-hand runs.
// The generated text goes through this file rather than straight into the repo on purpose: `final`
// and `decision` are derived here, the length cap lives here, and a model that ran long has to be
// rejected by the same rule regardless of which caller paid for it.
//
// Until 2026-08-11 this header said the site never calls Anthropic and never pays for a token. That
// was true and is no longer: the ~$1-per-case figure behind that decision turned out to be the
// Claude Code harness rather than the model, and the same prompts through the plain API cost about
// two cents. Since 2026-08-30 most cases cost nothing again - Google's free tier writes them and
// Anthropic catches what it drops. See lib/unbanOpinionsGenerator.js for the measurements.
//
...
// length-capped and stripped of control characters here, and rendered through textContent
// client-side.
//
...
// Measured, not guessed. A live run of the then-frozen prompt on the `onecrippled` case produced
// 724 / 691 / 482 characters against a prompt asking for "до 60 слов" - i.e. the accusation alone
// would have failed the 700-char cap this was originally specced at. Whatever the number, this has
// always been a backstop against a model running away with itself, not a formatting rule.
//
// RAISED 1200 -> 2000 ON 2026-08-30, because at 1200 it had started cutting real speeches instead
// of runaway ones. The prompt files stopped naming a length ("длина по делу") and both vendors
// then wrote past 1200 on a live brief - Gemini at 1850-2190, Sonnet at 1349 - each time on a case
// that had that much in it. Nothing downstream pays for the extra room: the sheet scrolls
// (`#ub-experts-body` in public/css/unban-bureau.css), so a long speech costs its reader some
// scrolling and nothing else.
//
// IT IS ALSO A PROMPT. A speech that overran this reached the moderator as a failed press and a
// blank sheet, so the generator states the ceiling to the models - a little under this number, to
// leave room for the overshoot a model given its exact limit produces - and rejects an over-long
// speech itself, retrying the other vendor. That is the instruction; this is still the guarantee,
// and it is why a hand-written PUT is held to the same limit as a generated one.
