...
function parseGlobalConfig(body, existing) {
  return {
    ...
    // Пустая строка - осмысленное значение: «вернуть встроенные правила». Поэтому здесь обычный
    // trimTo, а не «пусто значит не трогать». `builtinSystemPrompt` НЕ разбирается: его пишет
    // бот, и форма не имеет права его перезаписать.
    systemPrompt: trimTo(body.systemPrompt, MAX_SYSTEM_PROMPT_LEN),
  };
}
