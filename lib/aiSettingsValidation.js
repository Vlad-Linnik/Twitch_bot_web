...
const BOUNDS = {
  ...
  // Every remembered fact is re-sent on every billed call for that channel, so this bound is a
  // spend bound: at the top of the range a channel's memory is a few thousand input tokens on
  // every single answer. Zero is legitimate - it leaves the memory switched off without touching
  // anything else.
  channelMemoryMax: { min: 0, max: 100 },
