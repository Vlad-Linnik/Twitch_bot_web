const { parseUnbanBureau } = require("./unbanBureauValidation");
...
function parseSubmittedConfig(body, existing) {
  ...
  for (const [name, existingCmd] of Object.entries(existing.commands || {})) {
    ...
    // insult.cumulativeDelayMs - the window inside which repeated banned words stack their
    // timeout. Edited in seconds like the cooldowns, and only from the admin panel: it moved off
    // the channel settings page along with the other fine numbers.
    if (existingCmd.cumulativeDelayMs !== undefined) {
      const seconds = parseCooldownSeconds(body[`commands.${name}.cumulativeDelaySeconds`]);
      if (seconds !== null) commands[name].cumulativeDelayMs = seconds * 1000;
    }
    ...
    // Unlike signature, a blank defaultCity is a meaningful state (falls back to the
    // bot's "please specify a city" usage message) rather than something that would
    // break trigger matching - so, like the responses lists below, this one clears on
    // an empty submission instead of following the signature fields' blank-keeps rule.
    // It must therefore distinguish "" (the field was rendered and cleared) from absent
    // (the submitting page never rendered it): the field only exists on the main
    // settings form, so an unconditional overwrite silently wiped the stored city on
    // every save from the custom-commands / counters / unban-bureau sub-pages. Every
    // other field here is already absence-safe - signatures keep on blank, cooldowns
    // parse to null-means-keep, toggles need their `.present` marker.
    const submittedCity = body[`commands.${name}.defaultCity`];
    if (existingCmd.defaultCity !== undefined && submittedCity !== undefined) {
      commands[name].defaultCity = sanitizeWord(submittedCity);
    }
  }
  ...
  return {
    ...
    commands,
    // Same partial-form contract as everything else here: this whole block carries over unchanged
    // when the submitting page doesn't render it. It has to be listed explicitly - this function
    // builds a fixed shape rather than spreading `existing`, so a block omitted here is a block
    // erased from the channel's stored config on the next save from any page.
    unbanBureau: parseUnbanBureau(body, existing.unbanBureau),
    responses: {
