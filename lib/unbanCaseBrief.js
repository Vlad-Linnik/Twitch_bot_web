...
// How much log a case is WORTH showing, decided once up front by db/unbanDossierRepo.js's
// getCaseBrief() (via pickLogBudget below) rather than left at the same size for every case or
// handed to the agents to ask for more themselves - the owner explicitly wanted a fixed, predictable
// cost rather than a per-case tool-use loop (each extra fetch would be an extra paid model turn).
// EXPANDED is a flat ceiling, not a multiplier on the case's real history: a case with 5 past
// punishments and one with 200 get the same expanded budget, so cost never scales with how bad a
// record actually is - the whole point is that it can never turn into reading the full history.
const LOG_BUDGET = {
  base: { recentMessages: 30, logLimit: 40, perAction: 10, maxActions: 5 },
  expanded: { recentMessages: 50, logLimit: 60, perAction: 10, maxActions: 10 },
};
// A case earns the expanded budget on either signal alone: a real history of punishments (more than
// a couple of isolated incidents - the "иногда есть что сказать" case) or an appeal detailed enough
// to actually check against the log (the "иногда все очевидно" case, inverted - a one-line "прошу
// амнистии" appeal has nothing in it worth checking, however long the punishment history is). Neither
// threshold is tuned against real data yet; both are round numbers meant to separate "obvious, thin
// case" from "actually worth the extra log", not a calibrated cutoff.
const EXPANDED_PUNISHMENT_THRESHOLD = 5;
const EXPANDED_APPEAL_CHARS = 300;

/**
 * Picks how much log a case gets, from signals already sitting on the case document - no extra
 * query, so this costs nothing to call. `unbanCase.twitchModLogs.counts` (Twitch's real lifetime
 * totals, mirrored by the bot) is the only source used for the punishment count on purpose: the
 * bot-only fallback total undercounts old accounts so badly (see unbanDossierRepo.js's own comment -
 * a user Twitch counted at 17 bans had 0 on our own record) that trusting it here would size the
 * budget off a number already known to be wrong more often than not. A case with no mirror just gets
 * the base budget - the conservative default, not a penalty.
 *
 * @param {object} unbanCase  the raw UnbanRequests document
 * @returns {{recentMessages: number, logLimit: number, perAction: number, maxActions: number}}
 */
function pickLogBudget(unbanCase) {
  const counts = unbanCase?.twitchModLogs?.counts;
  const punishmentCount = counts ? (counts.bans || 0) + (counts.timeouts || 0) + (counts.warns || 0) : 0;
  const appealLength = String(unbanCase?.text ?? "").trim().length;
  const expanded = punishmentCount >= EXPANDED_PUNISHMENT_THRESHOLD || appealLength >= EXPANDED_APPEAL_CHARS;
  return expanded ? LOG_BUDGET.expanded : LOG_BUDGET.base;
}
