...
// Minimum 5-minute buckets (per-minute message counts were too spiky to read as a trend line),
// widened further only to keep the message-rate chart's point count under maxPoints regardless
// of how long the stream ran - a 12-hour "IRL" stream must not force the chart to render 720+
// points when 400 read exactly as well.
function messageBucketMs(durationMs, maxPoints) {
  const oneMinute = 60000;
  const minBucketMinutes = 5;
  if (durationMs <= 0) return minBucketMinutes * oneMinute;
  const minutesNeeded = Math.ceil(durationMs / maxPoints / oneMinute);
  return Math.max(minBucketMinutes, minutesNeeded) * oneMinute;
}
