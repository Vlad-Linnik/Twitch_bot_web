...
//
//     Return `truncated: true` when the driver's own work cap stopped it
//     before the end of the log (the same meaning deadlineExceeded() has,
//     for a limit the driver rather than the caller owns). The returned score
//     is then a partial one and is NOT compared against the claim - the run
//     degrades to unverified instead of being flagged for a shortfall the
//     driver itself caused.
//
...
// Findings that are worth RECORDING but never worth withholding a score for.
//
// A held score costs a real player their record until someone clicks, so the
// bar for holding is "this run might be worth more than it should be". Nothing
// in here can be: `scoreOverrun` means the re-simulation scored the run HIGHER
// than the player claimed, and the stored number is already clamped down to
// their own claim (see the score reconciliation below) - so publishing it
// hands over strictly less than the replay itself proves. It stays a flag
// because a client/server divergence is our bug to find, with the replay kept
// as the evidence; it is simply not the player's problem.
const OBSERVATIONAL_CODES = new Set(["scoreOverrun"]);

// Whether a set of findings justifies keeping the score off the leaderboard.
// One non-observational finding is enough - a run flagged for BOTH a divergence
// and a timing pattern is held on the timing.
function shouldHold(reasons) {
  if (!Array.isArray(reasons) || reasons.length === 0) return false;
  return reasons.some((r) => r && !OBSERVATIONAL_CODES.has(r.code));
}
...
function verify(input) {
  ...
  if (run.checkpointReplay) {
    try {
      ...
      if (head.gameKey === game && head.rulesVersion === decoded.rulesVersion && head.seed === decoded.seed) {
        // Trim the head to the watermark the CLIENT actually sliced its tail
        // at (`from`). The two can differ by one checkpoint: a checkpoint
        // request that lands here but whose response is still in flight when
        // the run ends is banked in the run document while the client's
        // watermark still points before it, so head and tail would overlap
        // and the overlapping stretch would be replayed - and scored - twice.
        // An older client sends no `from` at all; fall back to the whole head.
        const from = Number.isInteger(input.replayFrom) ? input.replayFrom : head.events.length;
        // The other direction is not repairable: a tail sliced past the end of
        // the head we hold means the events in between reached nobody, so the
        // stitched log has a hole in it. Replaying that would under-score an
        // honest player, so the run degrades to unverified instead.
        if (from > head.events.length) checkpointGap = true;
        const headEvents = head.events.slice(0, Math.max(0, Math.min(from, head.events.length)));
        // The tail's first dt was measured from the last event BEFORE the
        // slice, so the shift is that event's own timestamp - not the head's
        // full span, which may now include events we just trimmed off.
        const headSpanMs = headEvents.length ? headEvents[headEvents.length - 1].at : 0;
        const shiftedTail = decoded.events.map((ev) => ({ ...ev, at: ev.at + headSpanMs }));
        decoded = { ...decoded, events: headEvents.concat(shiftedTail) };
      }
    } catch {
  ...
  if (checkpointGap) {
    return {
      ...
      ...unverifiedOutcome({
        ...
        deathClimb,
        reasons: [{ code: "checkpointGap", detail: null }],
      }),
    };
  }
  ...
  if (timedOut || (result && result.truncated)) {
    // Both mean the same thing: the re-simulation stopped short of the end of
    // the log, so its score is not the run's score. Degrade to unverified
    // rather than compare a partial number against the player's claim.
    return {
      ...base,
      ...unverifiedOutcome({
        game,
        ...
        reasons: [
          timedOut
            ? { code: "replayTimeout", detail: { budgetMs: REPLAY_BUDGET_MS } }
            : { code: "replayTruncated", detail: result.detail || null },
        ],
      }),
    };
  }
