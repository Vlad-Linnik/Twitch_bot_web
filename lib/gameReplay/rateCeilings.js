...
//   SCORE_MULTIPLIER = 8 (:176), MONSTER_KILL_BONUS = 25 (:97)
//   The game's score line is
//     score = (floor(climbPx / 10) + bonusScore) * 8
//   i.e. climb buys a point per WHOLE 10px, not 0.8 per px. Averaged over a
//   run the two are the same 0.8 px^-1 rate, which is what the per-second
//   ceilings below are built on - but they are NOT the same number for one
//   given climb, and the exact identity is the one the equality check needs.
...
const CLOUD_CLIMBER = {
  ...
  maxKillsPerSec: 1.8,
  // Averaged rate - for the per-second ceilings only. Never for an equality:
  // use climbPerPoint/scoreMultiplier below.
  scorePerPx: 0.8,
  scorePerKill: 200,
  climbPerPoint: 10,
  scoreMultiplier: 8,
...
// The floor the game's own score line guarantees for a given climb. Exact, not
// approximate: `bonusScore` is a count of monster kills and can only add, so a
// run's score is never below this and any shortfall is a real disagreement.
//
// Using 0.8*deathClimb here instead is off by up to 7 points - the remainder
// the game's floor() throws away - which is a shortfall on ~4 runs in 5 and
// made climbMismatch a pure false-positive generator.
function climbScoreFloor(deathClimb) {
  return CLOUD_CLIMBER.scoreMultiplier * Math.floor(deathClimb / CLOUD_CLIMBER.climbPerPoint);
}
...
function check({ game, score, elapsedMs, eventCount, deathClimb, untokened }) {
  ...
  if (game === "cloud-climber" && Number.isFinite(deathClimb) && deathClimb > 0) {
    const expectedAtLeast = climbScoreFloor(deathClimb);
    if (score < expectedAtLeast) {
      reasons.push({
        code: "climbMismatch",
        detail: { score, deathClimb, expectedAtLeast },
      });
    }
...
module.exports = { CEILINGS, CLOUD_CLIMBER, LEGACY_CAP, MIN_ELAPSED_MS, MIN_SCORE, climbScoreFloor, check };
