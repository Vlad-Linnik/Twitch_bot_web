...
function replay(events, seed, ctx) {
  ...
  function advanceTo(target) {
    while (!state.over && simMs + engine.FIXED_DT_MS <= target) {
      if (steps >= MAX_STEPS) {
        // Past this point the replay stops advancing gravity but keeps
        // applying the player's inputs, so whatever it scores from here is
        // meaningless. Report it: a marathon longer than MAX_STEPS is a real
        // possibility (~40 minutes) and silently under-scoring one would flag
        // an honest player for claiming more than the replay produced.
        truncated = true;
        return;
      }
