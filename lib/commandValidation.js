...
// Free-text label a mod can assign a command to, purely for sorting/bulk-enable on this page -
// unrelated to categoryTexts above despite the similar-sounding name (that's the stream's Twitch
// category; this is a mod-invented grouping label, deliberately called "group" in the UI to keep
// the two apart). Web-panel-only, same as categoryTexts/modOnly - no chat command sets it.
const MAX_GROUP_LENGTH = 40;

// Group assignment is no longer part of the create/edit form - it's its own action now (the row's
// "..." menu and drag-and-drop, routes/customCommands.js's "setGroup"), so it needs to be
// validatable on its own rather than only as one field of a whole parsed command. An empty string
// is legal and means "ungrouped" - that's how a command is taken back out of a group.
function parseGroupName(raw) {
  const group = String(raw ?? "").trim();
  if (group.length > MAX_GROUP_LENGTH) return { ok: false, error: "group_too_long" };
 ...
 */
function parseCommand({ name, result, timerSeconds, pin, announce, announceColor, enabled, categoryTexts, modOnly, aliases, group }) {
  ...
  const groupResult = parseGroupName(group);
  if (!groupResult.ok) return groupResult;
  const groupName = groupResult.group;
  ...
  return {
    ...
    command: {
      ...
      // wantsModOnly used to be computed above and then never placed here - the "mods only"
      // checkbox silently had no effect on save. See routes/customCommands.js's save() call.
      enabled: wantsEnabled, categoryTexts: categoryResult.categoryTexts, modOnly: wantsModOnly,
      aliases: aliasResult.aliases, group: groupName,
    },
  };
}
