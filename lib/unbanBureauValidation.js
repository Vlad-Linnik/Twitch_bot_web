...
// The grenade's own bounds. Its punishment is clamped exactly like the rifle's (same Twitch
// endpoint, same limits), but its cooldown has a FLOOR rather than just a range: the cooldown is
// the only thing standing between one click and a chat wiped every two seconds, so a channel may
// lengthen the wait and never remove it. Five minutes is roughly how long a 30-second blast stays
// visible in chat - long enough that a second one is a new decision rather than a continuation.
const MIN_GRENADE_COOLDOWN_SEC = 300;
const MAX_GRENADE_COOLDOWN_SEC = 24 * 3600;
...
function parseUnbanBureau(body, existing) {
  ...
  return {
    ...
    // There is no `sniperEnabled`. The sniper is fired BY HAND from the review console and nowhere
    // else - the bot never shoots on its own - so a moderator pulling the trigger already IS the
    // decision a toggle would have been asking about. (Until 2026-08-08 there was one, left over
    // from an earlier version that fired automatically whenever a chat vote closed; the automatic
    // path is long gone, and the toggle survived only as a switch that silently made the rifle do
    // nothing.) The three fields below describe what happens to whoever that manual shot hits.
    //
    ...
    // The grenade takes everyone who spoke in the last 30 seconds instead of one drawn victim, so
    // it gets its own punishment rather than sharing the rifle's: the sensible timeout for a dozen
    // people at once is not the one you would pick for a single joke victim.
    grenadeMode:
      body["unbanBureau.grenadeMode"] !== undefined
        ? body["unbanBureau.grenadeMode"] === "ban"
          ? "ban"
          : "timeout"
        : current.grenadeMode,
    grenadeTimeoutSec:
      body["unbanBureau.grenadeTimeoutSec"] !== undefined
        ? clampInt(
            body["unbanBureau.grenadeTimeoutSec"],
            MIN_SNIPER_SEC,
            MAX_SNIPER_SEC,
            current.grenadeTimeoutSec
          )
        : current.grenadeTimeoutSec,
    grenadeReason:
      body["unbanBureau.grenadeReason"] !== undefined
        ? String(body["unbanBureau.grenadeReason"] ?? "").trim().slice(0, MAX_REASON_LEN) ||
          DEFAULTS.grenadeReason
        : current.grenadeReason,
    // Clamped rather than validated-with-an-error on purpose, like every other number on this form.
    // The floor is what makes the weapon un-spammable, so an unparseable value must not fall back
    // to something permissive: clampInt's fallback is the channel's CURRENT setting, which was
    // itself clamped when it was written.
    grenadeCooldownSec:
      body["unbanBureau.grenadeCooldownSec"] !== undefined
        ? clampInt(
            body["unbanBureau.grenadeCooldownSec"],
            MIN_GRENADE_COOLDOWN_SEC,
            MAX_GRENADE_COOLDOWN_SEC,
            current.grenadeCooldownSec
          )
        : clampInt(
            current.grenadeCooldownSec,
            MIN_GRENADE_COOLDOWN_SEC,
            MAX_GRENADE_COOLDOWN_SEC,
            DEFAULTS.grenadeCooldownSec
          ),
  };
}
